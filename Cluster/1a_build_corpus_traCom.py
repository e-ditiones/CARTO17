...
listPlaces = "traCom_listPlaces.tsv"
...
listPlaces_simple = "traCom_listPlaces_simple.tsv"
...
def append_list_as_row(list_of_elem):
    ...
    with open(os.path.join(dir_path,'listPlaces',"traCom_listPlaces.tsv"), 'a+', newline='') as write_obj:
...
def append_list_as_row_simple(list_of_elem):
    # Open file in append mode
    with open(os.path.join(dir_path,'listPlaces',"traCom_listPlaces_simple.tsv"), 'a+', newline='') as write_obj:
        # Create a writer object from csv module
        csv_writer = writer(write_obj, delimiter='\t')
        # Add contents of list as last row in the csv file
        csv_writer.writerow(list_of_elem)
...
for filename in os.listdir(os.path.join(dir_path,'data')):
	...
	if subgenre == "tragedy" or subgenre == "comedy":
...
data2 = pd.read_csv(os.path.join(dir_path,'listPlaces','traCom_listPlaces.tsv'), names=colnames, delimiter='\t', skiprows=1)
#join with the dictionary of name+coordinates
data2['coord'] = data2['placeName'].map(placeDict)
#save this new result
data2.to_csv(os.path.join(dir_path,'listPlaces','traCom_listPlaces_plus.tsv'), sep='\t', encoding='utf-8')

data3 = pd.read_csv(os.path.join(dir_path,'listPlaces','traCom_listPlaces_simple.tsv'), names=colnames, delimiter='\t', skiprows=1)
#join with the dictionary of name+coordinates
data3['coord'] = data3['placeName'].map(placeDict)
#save this new result
data3.to_csv(os.path.join(dir_path,'listPlaces','traCom_listPlaces_plus_simple.tsv'), sep='\t', encoding='utf-8')
